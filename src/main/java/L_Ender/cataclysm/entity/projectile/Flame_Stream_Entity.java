...
import L_Ender.cataclysm.client.model.tool.ControlledAnimation;
...
public class Flame_Stream_Entity extends Entity {
    ...
    private static final EntityDataAccessor<Float> YAW = SynchedEntityData.defineId(Flame_Stream_Entity.class, EntityDataSerializers.FLOAT);

    private static final EntityDataAccessor<Float> PITCH = SynchedEntityData.defineId(Flame_Stream_Entity.class, EntityDataSerializers.FLOAT);

    private static final EntityDataAccessor<Integer> DURATION = SynchedEntityData.defineId(Flame_Stream_Entity.class, EntityDataSerializers.INT);

    private static final EntityDataAccessor<Boolean> HAS_PLAYER = SynchedEntityData.defineId(Flame_Stream_Entity.class, EntityDataSerializers.BOOLEAN);

    private static final EntityDataAccessor<Integer> CASTER = SynchedEntityData.defineId(Flame_Stream_Entity.class, EntityDataSerializers.INT);
    ...
    public Flame_Stream_Entity(EntityType<? extends Flame_Stream_Entity> type, Level world) {
        super(type, world);
        noCulling = true;
    ...
    public Flame_Stream_Entity(EntityType<? extends Flame_Stream_Entity> type, Level world, LivingEntity caster, double x, double y, double z, float yaw, float pitch, int duration) {
        this(type, world);
        this.caster = caster;
        this.setYaw(yaw);
        this.setPitch(pitch);
        ...
        this.playSound(ModSounds.IGNIS_IMPACT.get(), 2f, 1);
        if (!world.isClientSide) {
        ...
        if (tickCount > 20) {
            ...
            if (!level.isClientSide) {
                for (LivingEntity target : hit) {
                    if (caster instanceof Ignis_Entity ) {
                        continue;
                    }
                    float damageFire = 1f;
