...
public class SoundLeviathanMusic extends AbstractTickableSoundInstance {
    private final The_Leviathan_Entity Guardian;
    private int ticksExisted = 0;
    public SoundLeviathanMusic(The_Leviathan_Entity guardian) {
        super(ModSounds.LEVIATHAN_MUSIC.get(), SoundSource.RECORDS, guardian.getRandom());
    ...
    public boolean shouldPlaySound() {
        return !this.Guardian.isSilent() && ClientProxy.LEVIATHAN_SOUND_MAP.get(this.Guardian.getId()) == this;
    }

    public boolean isNearest() {
        float dist = 400;
        for(Map.Entry<Integer, SoundLeviathanMusic> entry : ClientProxy.LEVIATHAN_SOUND_MAP.entrySet()){
            SoundLeviathanMusic GuardianMusic = entry.getValue();
    ...
    public void tick() {
        ...
        if (!this.Guardian.isRemoved() && this.Guardian.isAlive()) {
            ...
            this.stop();
            ClientProxy.LEVIATHAN_SOUND_MAP.remove(Guardian.getId());
        }
        ticksExisted++;
    }
